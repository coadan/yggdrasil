...
(def status-schema
  "agraph.map.status/v1")

(def review-schema
  "agraph.map.review/v1")
  ...
  (let [project-id (:id project)
    ...
    {:schema review-schema
     ...
     :nextActions [{:kind "review"
                    ...
                    :command "agraph map accept system <target> --kind KIND --label LABEL --include repo:path --reason TEXT --map agraph.map.json"}
                   {:kind "reject"
                    :label "Reject a known false positive"
                    :command "agraph map reject <kind> <value> --reason TEXT --map agraph.map.json"}]}))
