...
(defn api-key
  ...
  []
  (env/get-env "AGRAPH_OPENAI_API_KEY" "OPENAI_API_KEY"))
...
(defn embed-batch
  ...
  (when-not (seq inputs)
    [])
  (when-not api-key
    (throw (ex-info "Missing OpenAI API key. Set AGRAPH_OPENAI_API_KEY or OPENAI_API_KEY."
                    {:provider :openai})))
