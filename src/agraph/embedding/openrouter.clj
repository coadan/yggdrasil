(ns agraph.embedding.openrouter
...
(defn api-key
  ...
  []
  (env/get-env "AGRAPH_OPENROUTER_API_KEY" "OPENROUTER_API_KEY" "OPEN_ROUTER_API_KEY"))
...
(defn embed-batch
  ...
  (when-not (seq inputs)
    [])
  (when-not api-key
    (throw (ex-info "Missing OpenRouter API key. Set AGRAPH_OPENROUTER_API_KEY or OPENROUTER_API_KEY."
                    {:provider :openrouter})))
