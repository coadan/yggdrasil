(ns agraph.benchmark-claim-pack
...
(defn markdown-report
  ...
  (let [summary (:summary claim-pack)
        ...
        agraph-token (get-in claim-pack [:lanes :agraph :tokenTelemetry])
        tradeoff (:qualityCostTradeoff summary)
        artifacts (:artifacts claim-pack)]
    (str/join
     "\n"
     (concat
      ["# AGraph Benchmark Claim Pack"
       ""
       ...
       (str "- AGraph token telemetry: " (format-token-status agraph-token))]
...
(defn write-claim-pack!
  "Read shell-only and AGraph agent reports, then write claim-pack artifacts."
  [suite opts]
  (let [shell-report-path (required-path opts :shell-report "--shell-report")
        agraph-report-path (required-path opts :agraph-report "--agraph-report")
        shell-report (read-agent-report shell-report-path)
        agraph-report (read-agent-report agraph-report-path)
