(ns agraph.hook
  "Git hook installation for keeping AGraph refreshed."
...
(defn- sync-command
  [{:keys [agraph-bin config-path map-path query-index? repo-id]}]
  (str (shell-quote (or agraph-bin "agraph"))
       " sync "
...
(defn- hook-section
  ...
  (str begin-marker "\n"
       "AGRAPH_HOOK_LOG_DIR=\".dev/agraph/hooks\"\n"
       "mkdir -p \"$AGRAPH_HOOK_LOG_DIR\" 2>/dev/null || true\n"
       "AGRAPH_HOOK_LOG=\"$AGRAPH_HOOK_LOG_DIR/" repo-id "-$(date +%Y%m%d%H%M%S).log\"\n"
       "if [ -z \"${AGRAPH_HOOK_RUNNING:-}\" ]; then\n"
       "  AGRAPH_HOOK_RUNNING=1 " (sync-command opts) " >\"$AGRAPH_HOOK_LOG\" 2>&1 || true\n"
       "fi\n"
       end-marker "\n"))
...
(defn install!
  "Install AGraph refresh blocks into Git hooks for every Git repo in project."
  [project opts]
...
(defn uninstall!
  "Remove AGraph-owned Git hook blocks for every repo in project."
  [project]
  {:schema schema
   :action "uninstall"
   :project-id (:id project)
