...
(def install-schema
  "ygg.plugin.install/v1")

(def list-schema
  "ygg.plugin.list/v1")

(def remove-schema
  "ygg.plugin.remove/v1")

(def update-schema
  "ygg.plugin.update/v1")

(def new-schema
  "ygg.plugin.new/v1")

(def validate-schema
  "ygg.plugin.validate/v1")

(def dry-run-schema
  "ygg.plugin.dry-run/v1")

(def input-sample-schema
  "ygg.plugin.input-sample/v1")

(def extractor-gap-schema
  "ygg.plugin.extractor-gap/v1")

(def report-gap-schema
  "ygg.plugin.report-gap/v1")

(def diagnose-schema
  "ygg.plugin.diagnose/v1")

(def core-check-schema
  "ygg.plugin.core-promotion-check/v1")

(def registry-schema
  "ygg.plugin.registry/v1")

(def registry-validate-schema
  "ygg.plugin.registry.validate/v1")

(def registry-list-schema
  "ygg.plugin.registry.list/v1")

(def registry-install-schema
  "ygg.plugin.registry.install/v1")
    ...
    (when (commercial? package)
      [(if (= :public visibility)
         {:code :public-commercial
          :severity :error
          :applies-to [:public-sharing :claims :core-promotion]
          :message (str id " is marked public and commercial; public Yggdrasil/Yggdrasil plugins should be non-commercial.")
          :evidence {:visibility visibility
...
(defn- public-claim-requirements
  ...
  ["Keep output non-authoritative until benchmark artifacts exist."
   ...
   "Compare shell-only, core Yggdrasil, and plugin-enhanced Yggdrasil when claiming agent improvement."])
...
(defn- extractor-output-contract
  ...
  {:schema extractor-plugin/result-schema
   ...
   :non-goals ["No ownership, system-boundary, or runtime-criticality claims without map/metadata acceptance."
               "No project-specific logic belongs in Yggdrasil core without benchmarks and review."]})
