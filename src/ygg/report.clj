(ns ygg.report
  "Human-readable local report bundles for Yggdrasil projects."
...
(defn- ygg-home
  []
  (or (non-empty-string (System/getProperty "ygg.home"))
      (non-empty-string (System/getenv "YGG_HOME"))))
...
(defn- boot-script
  [boot]
  (str "<script>window.__YGG_BOOT__="
...
(defn- plugin-package-diagnose-command
  ...
  (when-let [path (:path package)]
    (str "ygg plugin diagnose " (command/shell-token path) " --json")))
...
(defn- package-command
  [project & args]
  (str "ygg packages --project " (command/shell-token (:id project))
       (when (seq args)
         (str " " (str/join " " (map command/shell-token args))))))

(defn- sync-command
  [project & args]
  (str "ygg sync " (command/shell-token (or (:path project) "<project.edn>"))
       (when (seq args)
         (str " " (str/join " " (map command/shell-token args))))))

(defn- sync-subcommand
  [project subcommand & args]
  (str "ygg sync " subcommand " " (command/shell-token (or (:path project) "<project.edn>"))
       (when (seq args)
         (str " " (str/join " " (map command/shell-token args))))))
  ...
  (let [package-counts (:counts package-report)
    ...
    (cond-> []
      ...
      (conj {:kind :maintenance
             ...
             :command (str "ygg sync work list --project "
...
(defn- maintenance-work-commands
  ...
  (let [queue (queue-summary maintenance)
    ...
    (cond-> []
      (some pos? (vals queue))
      (conj (str "ygg sync work list --project " (command/shell-token project-id))
