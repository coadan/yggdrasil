(ns ygg.watch
  "Filesystem watching helpers for refreshing Yggdrasil."
...
(defn watchable-path?
  "Return true when a repo-relative path should trigger Yggdrasil sync."
  [path]
...
(defn refresh!
  "Run one watch-triggered Yggdrasil sync. Returns process data."
  [{:keys [ygg-bin] :as opts}]
  (let [argv (into [(or ygg-bin "ygg")] (sync-args opts))]
    (apply shell/sh argv)))
