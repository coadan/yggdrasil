...
(defn- instruction-section
  ...
  (str begin-marker "\n"
       ...
       "ygg audit-scope <project.edn> --map ygg.map.json --json\n"
       ...
       "CLI-only inventory.\n\n"
       "When coverage, sync inspect, or answerability points at skipped or unsupported "
       ...
       "```sh\n"
       "ygg explore \"<question>\" --project <project-id> --json\n"
       "```\n\n"
       "For longer investigations with stable graph context:\n\n"
       "```sh\n"
       "ygg explore create \"<question>\" --project <project-id> --map ygg.map.json\n"
       "ygg explore open <cursor-id> <target-id-or-label>\n"
       "ygg explore expand <cursor-id> <target-id-or-label>\n"
       "ygg explore docs <cursor-id> <target-id-or-label>\n"
       "ygg explore search <cursor-id> \"<follow-up query>\"\n"
       "```\n\n"
       ...
       "```\n\n"
       "For MCP clients, run `ygg-mcp --config project.edn --map ygg.map.json` "
       "and start with the default typed tools: `ygg_explore`, "
       ...
       "Use `ygg_explore` as the primary one-shot orientation packet for "
       "structural questions. It returns the same context shape as `ygg ask` "
       "with graph-basis freshness, `answerability.planes`, candidate files, "
       ...
       "or `answerability` reports `limited`, `empty`, `stale`, or `unsynced`, follow "
       ...
       "packet evidence before applying accepted corrections to `ygg.map.json`.\n\n"
       ...
       "substring lists. Record accepted project meaning in `ygg.map.json` or "
       "metadata through supported `ygg sync` commands.\n\n"
  ...
  {:matcher "Bash"
   :hooks [{:type "command"
            :command (str
                      ...
                      "Prefer `ygg sync inspect <project.edn> --map ygg.map.json --json` "
                      "to inspect freshness and evidence.families, then "
                      "`ygg explore \\\"<question>\\\" --project "
