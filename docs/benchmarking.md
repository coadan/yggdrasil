...
the service is unavailable, indexing, or embedding:
...
`server-unavailable` filesystem packets. The active-work MCP lanes must return
`active-indexing` or `active-embedding`, retain the accepted repository scope,
