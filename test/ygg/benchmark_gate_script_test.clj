...
(deftest dry-run-prints-full-strict-gate
  (let [result (run-gate "--dry-run"
    ...
    (is (str/includes? (nth lines 0)
                       "clojure -M:run -m ygg.cli-bench repos check --manifest benchmarks/custom-repos.edn --suite benchmarks/custom.edn"))
...
(deftest help-lists-check-only
  (let [result (run-gate "--help")]
    ...
    (is (str/includes? (:out result)
                       "Benchmark actions run through the local source tree"))
    (is (str/includes? (:out result)
                       "Use --check-only before a claim"))))
...
(deftest claim-quick-dry-run-uses-claim-suite-and-expected-evidence-gates
  (let [result (run-claim-quick-gate "--dry-run")
    ...
    (is (= 4 (count lines)))
    (is (str/includes? (nth lines 0)
                       "clojure -M:run -m ygg.cli-bench repos check --manifest benchmarks/repos.edn --suite benchmarks/historical-replay-claim-quick.edn"))
    (is (str/includes? (nth lines 1)
...
(deftest claim-full-dry-run-uses-full-suite-and-strict-coverage-gates
  (let [result (run-full-claim-gate "--dry-run")
    ...
    (is (= 4 (count lines)))
    (is (str/includes? (nth lines 0)
                       "clojure -M:run -m ygg.cli-bench repos check --manifest benchmarks/repos.edn --suite benchmarks/historical-replay-full.edn"))
    (is (str/includes? (nth lines 1)
...
(deftest docs-claim-dry-run-uses-docs-suite-and-expected-evidence-gates
  (let [result (run-docs-claim-gate "--dry-run")
    ...
    (is (= 4 (count lines)))
    (is (str/includes? (nth lines 0)
                       "clojure -M:run -m ygg.cli-bench repos check --manifest benchmarks/repos.edn --suite benchmarks/historical-docs-claim-quick.edn"))
    (is (str/includes? (nth lines 1)
...
(deftest patch-claim-dry-run-checks-existing-patch-artifacts
  (let [result (run-patch-claim-gate "--dry-run"
    ...
    (is (str/includes? (nth lines 1)
                       "clojure -M:run -m ygg.cli-bench verifier-check benchmarks/oss-issue-patch-replay.edn"))
    (is (str/includes? (nth lines 2)
...
(deftest patch-claim-setup-check-only-runs-preflight
  (let [result (run-patch-claim-gate "--dry-run" "--setup-check")
    ...
    (is (str/includes? (second lines)
                       "clojure -M:run -m ygg.cli-bench verifier-check benchmarks/oss-issue-patch-replay.edn"))
    (is (not-any? #(str/includes? % "agent-check") lines))))
