...
(deftest cursor-create-open-and-expand-are-revisioned
  (store/with-node (temp-dir "ygg-cursor-xtdb")
    (fn [xtdb]
      (let [{:keys [api-id worker-id]} (seed-system-graph! xtdb)
        ...
        (is (some #(= {:kind :expand
                       ...
                       :target api-id
                       :command (str "ygg explore expand " cursor-id " " api-id)
                       :reason "Follow adjacent system graph edges"}
                      %)
                  (:nextActions created)))
        (is (some #(= {:command (str "ygg explore search " cursor-id " <text>")
...
(deftest cursor-docs-use-stored-map-overlay
  (let [map-path (.getPath (io/file (temp-dir "ygg-cursor-map") "ygg.map.json"))]
    (store/with-node (temp-dir "ygg-cursor-map-xtdb")
      (fn [xtdb]
...
(deftest invalid-target-does-not-create-revision
  (store/with-node (temp-dir "ygg-cursor-invalid-xtdb")
    (fn [xtdb]
      (seed-system-graph! xtdb)
      (let [created (cursor/create! xtdb
                                    {:project-id "test"
