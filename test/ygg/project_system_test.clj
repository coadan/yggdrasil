...
(deftest indexes-project-and-infers-system-graph
  (let [xtdb-path (temp-dir "ygg-project-xtdb")
    ...
    (store/with-node xtdb-path
      (fn [xtdb]
        (let [index-summary (project/index-project! xtdb project {})
          ...
          (is (= "ygg.graph-basis/v1" (get-in maintenance [:graph-basis :schema])))
...
(deftest maintenance-queues-bounded-noise-decisions
  (let [xtdb-path (temp-dir "ygg-maintenance-noise-xtdb")
      ...
      (fn [xtdb]
        ...
        (let [maintenance (project/maintain-project
          ...
          (is (= [{:kind :pull-work
                   ...
                   :command "ygg sync work pull --project noise --kind maintenance-decision --agent <agent-id>"}
...
(deftest container-image-artifacts-connect-producers-to-deployment-manifests
  (let [xtdb-path (temp-dir "ygg-image-xtdb")
        app-root (temp-dir "ygg-image-app")
        env-root (temp-dir "ygg-image-env")
        project {:id "images"
                 :name "Images"
...
(deftest unmatched-container-image-facts-create-infra-review-work
  (let [xtdb-path (temp-dir "ygg-image-review-xtdb")
        app-root (temp-dir "ygg-image-review-app")
...
(deftest shell-image-tags-connect-build-and-runtime-scripts
  (let [xtdb-path (temp-dir "ygg-shell-image-xtdb")
        repo-root (temp-dir "ygg-shell-image-repo")
...
(deftest system-inference-uses-indexed-file-facts
  (let [xtdb-path (temp-dir "ygg-indexed-facts-xtdb")
        repo-root (temp-dir "ygg-indexed-facts-repo")
...
(deftest context-packet-preserves-retrieval-rank-over-repetitive-token-overlap
  (store/with-node (temp-dir "ygg-context-retrieval-rank-xtdb")
    (fn [xtdb]
...
(deftest context-packet-balances-retrieved-source-docs-by-definition-kind
  (store/with-node (temp-dir "ygg-context-definition-kind-xtdb")
    (fn [xtdb]
...
(deftest adds-repo-to-project-config
  (let [root (io/file (temp-dir "ygg-add-repo"))
