...
(deftest agent-install-codex-project-guidance-is-idempotent
  (let [root (temp-dir "agraph-agent-install")
    ...
    (let [first-content (slurp agents)]
      ...
      (is (str/includes? first-content "agraph sync inspect <project.edn> --map agraph.map.json --json"))
      ...
      (is (str/includes? first-content "When coverage, sync inspect, or answerability points at skipped or unsupported"))
