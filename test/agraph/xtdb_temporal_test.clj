...
(deftest temporal-ops-read-valid-time-history
  (let [table :agraph/temporal-test
        id "fact:alpha"]
    (store/with-node (temp-dir "agraph-temporal-xtdb")
      (fn [xtdb]
        (store/execute-tx!
         xtdb
...
(deftest temporal-bundle-writes-snapshot-run-puts-and-deletes
  (store/with-node (temp-dir "agraph-temporal-bundle-xtdb")
    (fn [xtdb]
...
(deftest read-context-threads-through-query-and-graph
  (store/with-node (temp-dir "agraph-temporal-query-xtdb")
    (fn [xtdb]
