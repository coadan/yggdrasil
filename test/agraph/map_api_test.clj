...
(deftest map-store-normalizes-generated-candidate-dumps-to-compact-v2
  (let [dir (temp-dir "agraph-map-normalize")
...
(deftest map-store-write-strips-generated-candidates
  (let [dir (temp-dir "agraph-map-write-compact")
        map-path (.getPath (io/file dir "agraph.map.json"))
...
(deftest map-review-is-read-only-and-returns-candidates
  (let [dir (temp-dir "agraph-map-review")
        map-path (.getPath (io/file dir "agraph.map.json"))]
...
(deftest map-accept-system-writes-compact-correction-through-cli
  (let [dir (temp-dir "agraph-map-accept")
        map-path (.getPath (io/file dir "agraph.map.json"))]
...
(deftest map-writes-require-auditable-reason
  (let [dir (temp-dir "agraph-map-reason")
        map-path (.getPath (io/file dir "agraph.map.json"))]
    (map-store/write-map! map-path (graph-map/empty-map "fixture"))
    (try
...
(deftest sync-map-write-aliases-are-retired
  (try
    ...
    (catch clojure.lang.ExceptionInfo e
      (is (= "Map corrections are handled by the agraph map API."
             (.getMessage e)))
      (is (= "agraph map" (:replacement (ex-data e))))))
  (try
    ...
    (catch clojure.lang.ExceptionInfo e
      (is (= "Map doc attachments are handled by the agraph map API."
             (.getMessage e)))
      (is (= "agraph map docs attach" (:replacement (ex-data e))))))
  (try
    (cli/dispatch "docs" ["attach" "agraph.map.json" "API Gateway" "app:docs/api.md"
                          "--reason" "Reviewed API contract"])
  ...
  (try
    ...
    (catch clojure.lang.ExceptionInfo e
      (is (= "Package import corrections use map package import."
             (.getMessage e)))
      (is (= "agraph map package import" (:replacement (ex-data e)))))))
